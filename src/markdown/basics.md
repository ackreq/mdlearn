...
در مارکدون برای نمایش متن به صورت کد، چند روش وجود دارد: **کد خطی**، **بلوک کد**، و **بلوک کد حصاردار**
...
می‌توانید بلوک‌های کد را با سه بک‌تیک پشت سر هم ` ``` ` در ابتدا و انتهای کد مشخص کنید. این روش خوانایی متن را افزایش می‌دهد و مخصوصاً برای کدهای طولانی یا نمونه‌هایی که می‌خواهید هایلایت داشته باشند مناسب است.
...
بعد از بک‌تیک‌های اول می‌توانید **نام زبان برنامه‌نویسی** را بنویسید (مثلاً `python`) تا پردازشگر Markdown بتواند **هایلایت** مختص به آن زبان را اعمال کند.

```admonish tip title="نکته کاربردی"
برای کدهای کوتاه استفاده از بلوک‌های ساده کفایت می‌کند، اما برای کدهای طولانی یا زمانی که می‌خواهید هایلایت داشته باشید، بلوک حصاردار توصیه می‌شود.
```
...
برای ایجاد یک خط افقی (Horizontal Rule) که معمولاً برای جدا کردن بخش‌ها یا موضوعات استفاده می‌شود، کافی است سه علامت یا بیشتر از یکی از موارد آورده شده را در یک خط قرار دهید: ستاره (`*`)، خط تیره (`-`) یا زیرخط (`_`).
